...
def get_raw_claude_response(ocr_data, log_file):
    """获取Claude 3的原始输出"""
    client = Anthropic(
        api_key='',
        base_url='',
    ...
    try:
        response = client.messages.create(
            ...
            max_tokens=4000,
            system="你是一个OCR隐私信息提取器，请直接返回JSON格式结果",
...
def process_single_file(input_path, output_path, log_file):
    ...
    try:
        ...
        raw_output = get_raw_claude_response(ocr_data, log_file)
...
def process_json_files(input_dir="ocr/PICL4", output_dir="claudeprivacyjiu/PICL4"):
    """多线程处理目录下的所有JSON文件"""
